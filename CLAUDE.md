...
- **Change the seeded admin password** (`admin@dautaytoystore.vn` / `Admin@123456`) before
  deploying/sharing this environment for real — it was rotated once already this session, then
  reverted back to the seeded default at the project owner's request (locked out of the rotated
  one), so it's live on the shared Supabase DB again as of now.
